...
class SeasonalityEngine:
    ...
    def fetch_long_history(ticker: str, period: str = "10y", use_cache: bool = True, max_age_hours: float = 24) -> pd.DataFrame:
        ...
        try:
            if DataEngine.is_tw_code(ticker):
                ...
                if df.empty and last_err:
                    SeasonalityEngine._last_fetch_error = last_err
            else:
                df = yf.download(ticker, period=period, interval="1d", progress=False, auto_adjust=True)
        except Exception as e:
            SeasonalityEngine._last_fetch_error = f"{type(e).__name__}: {e}"
            return pd.DataFrame()

        if df is None or df.empty:
            if not SeasonalityEngine._last_fetch_error:
                SeasonalityEngine._last_fetch_error = "yfinance回應內容為空（連線成功但沒有資料，可能是代碼錯誤或上市時間過短）"
            return pd.DataFrame()
        ...
        if df.empty:
            SeasonalityEngine._last_fetch_error = "資料下載成功但清理後沒有有效收盤價"
            return df
