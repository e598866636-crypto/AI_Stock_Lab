...
class ChipEngine:
    ...
    def _fetch_institutional_single_day(date_str: str) -> pd.DataFrame:
        try:
            ...
            ChipEngine._last_fetch_error["institutional"] = f"{type(e).__name__}: {e}"
            return pd.DataFrame()
        ...
        if data.get("stat") != "OK" or not data.get("data"):
            ...
            ChipEngine._last_fetch_error["institutional"] = f"TWSE回應 stat={data.get('stat')}，說明：{data.get('stat')}"
            return pd.DataFrame()
    ...
    def get_market_wide_institutional_ranking(lookback_days: int = 10, top_n: int = 20,
        ...
        if work is None:
            for delta in range(lookback_days):
                ...
                if code_col is None:
                    ChipEngine._last_fetch_error["ranking"] = (
                ...
                if candidate.empty:
                    ChipEngine._last_fetch_error["ranking"] = f"{d.strftime('%Y-%m-%d')}: 過濾代碼格式後沒有剩下任何資料列"
                    continue
    ...
    def _fetch_shareholding_distribution_full(use_cache: bool = True, max_age_hours: float = 20) -> pd.DataFrame:
        ...
        try:
            ...
            ChipEngine._last_fetch_error["shareholding"] = f"{type(e).__name__}: {e}"
            return pd.DataFrame()

        if df.empty:
            ChipEngine._last_fetch_error["shareholding"] = "TDCC回應內容為空（連線成功但沒有資料，可能是TDCC服務端暫時異常）"
            return df
        ...
        if not expected_cols.issubset(set(df.columns)):
            ...
            ChipEngine._last_fetch_error["shareholding"] = (
    ...
    def _fetch_insider_holdings_full(use_cache: bool = True, max_age_hours: float = 20) -> pd.DataFrame:
        ...
        try:
            ...
            ChipEngine._last_fetch_error["insider"] = f"{type(e).__name__}: {e}"
            return pd.DataFrame()

        if df.empty:
            ChipEngine._last_fetch_error["insider"] = "TWSE回應內容為空（連線成功但沒有資料，可能是TWSE服務端暫時異常）"
            return df
        ...
        if not expected_cols.issubset(set(df.columns)):
            ...
            missing = expected_cols - set(df.columns)
            ChipEngine._last_fetch_error["insider"] = (
                f"欄位格式與預期不符，缺少欄位：{missing}，實際欄位：{list(df.columns)[:10]}"
            )
            return pd.DataFrame()
    ...
    def get_insider_holdings(stock_code: str, use_cache: bool = True) -> dict:
        ...
        total_insider_holding = int(match["內部人關係人目前持股合計"].max())
        ...
        # 容易漏掉風險。
        max_pledge_pct = max(individual_max_pledge_pct, relationship_max_pledge_pct)
        ...
        return {
            ...
            "max_pledge_pct": round(max_pledge_pct, 2),
