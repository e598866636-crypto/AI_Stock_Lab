...
class MacroEngine:
    ...
    @staticmethod
    def get_snapshot(period: str = "6mo") -> dict:
        """
        回傳每個總經代理指標的最新值、日變動%、5日變動%、20日變動%。
        任何一檔查詢失敗都不影響其他項目（防禦性設計，沿用本專案風格），
        該項目會標記 status='unavailable'，呼叫端需自行處理顯示。

        ⚠️ 沒有走 DatabaseEngine 快取（這些是每天只需要看一次的宏觀背景
        資料，且都是輕量單檔下載，不像個股K線那樣有數十檔重複查詢的
        效能問題）；若之後改成排程批次更新，可以再補上快取層。
        """
        results = {}
        for name, symbol in MacroEngine.TICKERS.items():
            df = MacroEngine._fetch_one(symbol, period=period)
            if df is None or len(df) < 2:
                results[name] = {"status": "unavailable", "symbol": symbol}
                continue
