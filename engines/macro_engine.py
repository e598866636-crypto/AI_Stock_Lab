...
class MacroEngine:
    ...
    def _fetch_one(symbol: str, period: str = "6mo"):
        try:
            ...
            if df.empty:
                MacroEngine._last_fetch_error[symbol] = "yfinance回應內容為空（連線成功但沒有資料）"
                return None
            ...
            if df.empty:
                MacroEngine._last_fetch_error[symbol] = "資料下載成功但清理後沒有有效收盤價"
                return None
            MacroEngine._last_fetch_error.pop(symbol, None)
            return df
        except Exception as e:
            MacroEngine._last_fetch_error[symbol] = f"{type(e).__name__}: {e}"
            return None
