...
class OptionsEngine:
    ...
    def fetch_txo_put_call_ratio(use_cache: bool = True, max_age_hours: float = 4) -> pd.DataFrame:
        ...
        try:
            ...
            OptionsEngine._last_fetch_error = f"{type(e).__name__}: {e}"
            return pd.DataFrame()
        ...
        if df is None or df.empty:
            OptionsEngine._last_fetch_error = "頁面解析成功但找不到PCR表格（TAIFEX可能已改版頁面結構）"
            return pd.DataFrame()
        ...
        if not set(OptionsEngine._COLUMN_RENAME.keys()).issubset(set(df.columns)):
            ...
            OptionsEngine._last_fetch_error = f"欄位格式與預期不符，實際欄位：{list(df.columns)[:10]}"
            return pd.DataFrame()
