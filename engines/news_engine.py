...
class NewsEngine:
    ...
    def get_news_with_sentiment(code: str, name: str = None, max_items: int = None,
        ...
        + 概念股關聯 + 新聞量異動偵測 + 當日統計摘要 + AI摘要 + 達人視角
        使用提醒。
        ...
        'related_concepts'，並多以下欄位：
          - 'summary_stats': {'total','bullish','bearish','neutral',
        ...
        for item in news["items"]:
            ...
            raw_score = sentiment.get("weighted_score", 0)
            if raw_score > 0:
                weighted_bull += raw_score * recency_weight
            elif raw_score < 0:
                weighted_bear += abs(raw_score) * recency_weight

        bull_count = sum(1 for it in news["items"] if it["sentiment"]["label"] == "bullish")
        bear_count = sum(1 for it in news["items"] if it["sentiment"]["label"] == "bearish")
        ...
        news["summary_stats"] = {
            "total": total, "bullish": bull_count, "bearish": bear_count,
        ...
        ai_summary = None
        if llm_summarize_fn is not None and news["items"]:
            try:
                llm_result = llm_summarize_fn(news["items"], code, name)
        ...
        if ai_summary is None:
            ai_summary = {
                "bullets": NewsEngine._rule_based_stock_summary(news["items"]) if news["items"] else
                           ["今日無相關新聞可摘要。"],
                "label": overall_bias,
                "stars": NewsEngine._score_to_stars(weighted_bias_score),
                "method": "rule_based",
